...
class Test_x08:
    ...
    def test_channel_enables(self):
        ...
        for i in range(self.card_x08.no_of_channels):
            ...
            assert True == self.card_x08.channel_enables[i]

    def test_external_trigger_enable(self):
        assert False == self.card_x08.external_trigger_enable
        self.card_x08.external_trigger_enable = True
        assert True == self.card_x08.external_trigger_enable

    def test_firmware_version(self):
        assert 0 == self.card_x08.firmware_version
        self.card_x08.reset_registers()
        assert 1 == self.card_x08.firmware_version #this is the same register and therefore in emu will return what was written by reset_registers()

    def test_hardware_countdown(self):
        assert False == self.card_x08.hardware_countdown_enable
        self.card_x08.hardware_countdown_enable = True
        assert True == self.card_x08.hardware_countdown_enable
...
class Test_x04:
    ...
    def test_channel_enables(self):
        ...
        for i in range(4):
            ...
            assert False == self.card_x04.channel_enables[i]

    @pytest.mark.skipif(skip_test, reason = "Test Development")
    def test_external_trigger_enable(self):
        assert False == self.card_x04.external_trigger_enable
        self.card_x04.external_trigger_enable = True
        assert True == self.card_x04.external_trigger_enable

    @pytest.mark.skipif(skip_test, reason = "Test Development")
    def test_firmware_version(self):
        assert 0 == self.card_x04.firmware_version
        self.card_x04.reset_registers()
        assert 1 == self.card_x04.firmware_version #this is the same register and therefore in emu will return what was written by reset_registers()

    @pytest.mark.skipif(skip_test, reason = "Test Development")
    def test_hardware_countdown(self):
        assert False == self.card_x04.hardware_countdown_enable
        self.card_x04.hardware_countdown_enable = True
        assert True == self.card_x04.hardware_countdown_enable
    ...
    def test_cfd_thresholds(self):
        assert all(threshold == None for threshold in self.card_x04.cfd_thresholds)
    ...
    def test_cfd_zero_cross(self):
        assert all(zero_cross == None for zero_cross in self.card_x04.cfd_zero_cross)
    ...
    def test_dithering_enable(self):
        ...
        assert self.card_x04.dithering_enable == True
        self.card_x04.dithering_enable = False
        assert self.card_x04.dithering_enable == False

    def test_marker_enables(self):
        assert all(enable == False for enable in self.card_x04.marker_enables.values())
        ...
        self.card_x04.marker_enables = bh.Markers(pixel = True, line = False, frame = True, marker3 = False)
        ...
        for i, name in enumerate(markers):
            ...
            assert self.card_x04.marker_enables[name] == True
...
class Test_pms:
    ...
    def test_channel_enables(self):
        ...
        for i in range(self.card_pms.no_of_channels):
            ...
            assert False == self.card_pms.channel_enables[i]

    def test_external_trigger_enable(self):
        assert False == self.card_pms.external_trigger_enable
        self.card_pms.external_trigger_enable = True
        assert True == self.card_pms.external_trigger_enable

    def test_firmware_version(self):
        assert 0 == self.card_pms.firmware_version
        self.card_pms.reset_registers()
        assert 1 == self.card_pms.firmware_version #this is the same register and therefore in emu will return what was written by reset_registers()
    
    def test_hardware_countdown(self):
        assert False == self.card_pms.hardware_countdown_enable
        self.card_pms.hardware_countdown_enable = True
        assert True == self.card_pms.hardware_countdown_enable
